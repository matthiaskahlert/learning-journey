...
#### Theorie von Objekten
    ...
    So kann man auch im Code Testdaten abbilden, anstatt sie in unübersichtlichen Tabellen zu pflegen. Ich habe verstanden, dass Objekte eine Brücke zwischen Daten und logischer Verarbeitung bilden. 
...
### Verstehen und verknüpfen (Praxisbeispiel erklären)
    ...
    Dieses Beispiel verdeutlicht, wie man in einem Objekt sowohl Daten (z. B. health, position) als auch Verhalten über Methoden (z. B. move()) kombiniert. Über Methoden kann das Objekt eigenständig Zustände verändern. Das ist beispielsweise besonders in der Softwareentwicklung für Games wichtig.
    ...
    Das ist ganz ähnlich wie in einem Spiel, in dem jedes feature, Mechanik oder Entität in einem eigenen Kontainer organisiert ist und somit leichter getestet oder verändert werden kann.

    
    
### Anwenden und bewerten (berufliche Relevanz erörtern)

    Ich hatte es ja in den beiden Abschnitten zuvor schon erwähnt: Das Wissen über Funktionen und Modularisierung ist für meine berufliche Zukunft sehr relevant. 
    Im Testmanagement kann ich Funktionen theoretisch nutzen, um wiederkehrende Testschritte oder Prüfroutinen zu automatisieren. 
    ...
    Jede Funktion kann isoliert getestet und verifiziert werden, bevor sie in den Gesamtkontext eingebunden wird, dies hat direkten impact auf die gesamte Qualität der Software.
    ...
    Auch das Konzept der Objekte ist in meiner beruflichen Praxis von großer Bedeutung, da es helfen kann, Spielinhalte als auch Testdaten strukturiert abzubilden. 
...
### Reflektieren und Hinterfragen
    ...
    Hier möchte ich tiefer einsteigen und mehr in die Anwendung kommen, um zu verstehen, wie man solche Strukturen testbar und nachvollziehbar hält. 
    ...
    Ein weiteres Lernziel ist, die Verbindung zwischen Objekten und JSON und XML-Datenformaten zu vertiefen, da diese in der Testautomatisierung eine wichtige Rolle spielen. 
    ...
    Für die kommenden Tage meh,e ich mir vor, regelmäßig kleine Codebeispiele zu entwickeln, um Funktionen und Objekte gezielt zu kombinieren. 
