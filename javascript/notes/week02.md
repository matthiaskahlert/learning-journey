...
Zwei Themengebiete, welche mir derzeit als sehr relevant erscheinen sind Funktionen und Objekte.
 ...
 ### Einordnen und strukturieren (Theorie erklären)
    ...
    In der zweiten Weiterbildungswoche habe ich die Themengebiete Funktionen und Objekte als besonders interessant empfunden. 
    Funktionen sind zentrale Bausteine jeder Programmiersprache. 
    ...
    Damit habe ich die Grundlagen gelegt, um Code systematisch zu strukturieren, ein Prinzip, das sowohl in der Spieleentwicklung als auch im Testmanagement unverzichtbar ist.
    Objekte gehören zu den wichtigsten Konzepten in JavaScript, weil sie Daten und Verhalten gemeinsam abbilden können. 
    Sie bestehen aus sogenannten Schlüssel-Wert-Paaren, bei denen jeder Schlüssel (Property) einen bestimmten Wert (Value) enthält. 
    ...
    Ich habe verstanden, dass Objekte eine Brücke zwischen Datenhaltung und logischer Verarbeitung schlagen. 
...
### Verstehen und verknüpfen (Praxisbeispiel erklären)
    ...
    Dieses Beispiel verdeutlicht, wie man in einem Objekt sowohl Daten (z. B. health, position) als auch Verhalten (z. B. move()) kombiniert. Über Methoden kann das Objekt eigenständig Zustände verändern, ein Prinzip, das besonders in der Spielelogik wichtig ist.
    ...
    So wird deutlich, dass das theoretische Konzept von Objekten nicht abstrakt ist, sondern sich direkt auf reale Arbeitsprozesse übertragen lässt.
    Wenn Testabläufe oder Auswertungen modular aufgebaut sind, kann man sie leichter pflegen, erweitern oder automatisieren. 
    Das ist ganz ähnlich wie in einem Spiel, in dem jede Mechanik in einer eigenen Funktion organisiert ist und somit leichter getestet oder verändert werden kann.
...
### Reflektieren und Hinterfragen
    ...
    Außerdem interessiert mich, wie man Objekte in professionellen Testframeworks einsetzt, etwa wie Testfälle als Objekte zwischen verschiedenen Testebenen ausgetauscht werden können. 
