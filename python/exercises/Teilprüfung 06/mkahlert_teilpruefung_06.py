...
if __name__ == "__main__":
    ...
    laden.buch_hinzufuegen(Buch("Eine kurze Geschichte der Zeit", "Stephen Hawking", "Wissenschaft", 19.99))
    laden.buch_hinzufuegen(Buch("Mit Pflanzen die Welt retten", "Bernhard Kegel", "Sachbuch", 25.00))
    laden.buch_hinzufuegen(Buch("Natur auf dem Teller: Weil Pizza und Pommes nicht auf Bäumen wachsen", "Lisa Voisard", "Wissenschaft", 20.00))

    # Bücher nach Kategorie durchsuchen
    print("Bücher in der Kategorie 'Sachbuch':")
    sachbuch_buecher = laden.suche_nach_kategorie("sachbuch")
    for buch in sachbuch_buecher:
        print(f"  {buch}")

    print("\nBücher in der Kategorie 'Roman':")
    roman_buecher = laden.suche_nach_kategorie("Roman")
    for buch in roman_buecher:
        print(f"  {buch}")
